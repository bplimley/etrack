...
void RunAction::AddVerboseEventInformation(G4String VerboseEventInformation)
{
  
  //    G4cout << VerboseEventInformation << G4endl;
  
  EventInformationOut.append(VerboseEventInformation);
...
void RunAction::WriteVerboseEventInformation()
{
  // Write Summary EventInformation to File (StatisticsOut)
  G4String SumBaseName = "Sum_";
  
  char SumFileNameBase[250]="";
  // strcat (SumFileNameBase, savehitsFile);
  strcat (SumFileNameBase, SaveFilenameDir);
  strcat (SumFileNameBase, SumBaseName);
  strcat (SumFileNameBase, savehitsFile);
  
  G4cout << "Saving Output to: " << SumFileNameBase << G4endl;
  
  std::ofstream SimulationOutputFile;
  SimulationOutputFile.open(SumFileNameBase,std::ios::out | std::ios::trunc);
  SimulationOutputFile << StatisticsOut.str();
  SimulationOutputFile.close();
  
  //G4cout << EventInformationOut << G4endl;
  
  // Write VerboseEventInformation to File
  G4String MatBaseName = "Mat_";
  
  char StepFileNameBase[250]="";
  //strcat (StepFileNameBase, savehitsFile);
  strcat (StepFileNameBase, SaveFilenameDir);
  strcat (StepFileNameBase, MatBaseName);
  strcat (StepFileNameBase, savehitsFile);
  
  G4cout << "Saving Step Information to: " << StepFileNameBase << G4endl;
  
  std::ofstream SimulationStepsOutputFile;
  // SimulationStepsOutputFile.open(StepFileNameBase,std::ios::out | std::ios::trunc);
  SimulationStepsOutputFile.open(StepFileNameBase,std::ios::out | std::ios::app);
  //SimulationStepsOutputFile.write(EventInformationOut);
  SimulationStepsOutputFile << EventInformationOut;
  SimulationStepsOutputFile.close();
