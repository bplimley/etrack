...
:fRunAction(run)
{
  ...
  // Save Verbose Info: Set Tracking Matrix Save file...
  SaveHitsDir = new G4UIdirectory("/SimulationOutput/");
  SaveHitsDir->SetGuidance("Save Output to File");
  // File Location Base
  //G4UIcmdWithAString*   SaveFilenameBaseCmd;
  //G4UIdirectory*        SaveFilenameBaseDir;
  
  SaveFilenameBaseCmd = new G4UIcmdWithAString("/SimulationOutput/FileLocation",this);
  SaveFilenameBaseCmd->SetGuidance("Filename Directory Location for simulation output");
  SaveFilenameBaseCmd->SetGuidance("Default = /mnt/data/geant4/temp_data/");
  SaveFilenameBaseCmd->SetParameterName("SaveFilenameDir", false);
  SaveFilenameBaseCmd->SetDefaultValue("/mnt/data/geant4/temp_data/");
  
  // File Name Setting
  SaveHitsCmd = new G4UIcmdWithAString("/SimulationOutput/FileName",this);
  SaveHitsCmd->SetGuidance("Filename for simulation output");
  SaveHitsCmd->SetGuidance("Default = AbsorberAnalysis_000_01.output");
  SaveHitsCmd->SetParameterName("savehitsFile", false);
  SaveHitsCmd->SetDefaultValue("AbsorberAnalysis_000_01.output");
}
...
{      
  ...
  if(command == SaveFilenameBaseCmd)
    fRunAction->setSaveFilenameDir(newValue);
  
  if(command == SaveHitsCmd)
    fRunAction->SetsavehitsFile(newValue);
}
