...
def compute_deviation_factor_1996a(E_Mev, Z):
    E_Mev = float(E_Mev)
...
def compute_deviation_factor_2002(E_Mev, Z):
    E_Mev = float(E_Mev)
    t0 = E_Mev / ELECTRON_REST_ENERGY_MEV
...
def return_unity(E_Mev, Z):
    return 1.0


def extrapolated_range(Ekev, Z, A, I_EV, density, ref="2002"):

    def get_ref_mode(ref):
        if str(ref) == '2002':
            return '2002'
    ...
    def get_energy_range(refMode):
        ...
        if refMode == '2002':
            ...
            if any(Ekev > MIN_ENERGY and Ekev < SEMI_MIN_ENERGY):
                print("Range is less accurate below 10 keV, " +
                      "because shell effects are ignored.")
            if any(Ekev > MAX_ENERGY):
                print("Range is inaccurate above 100 MeV.")

    def get_deviation_function(refMode):
        # return function. google says you can do this.
        if refMode == '2002':
            return compute_deviation_factor_2002
        elif refMode == '1996a':
            return compute_deviation_factor_1996a
        elif refMode == 'CSDA':
            return return_unity
    ...
    refMode = get_ref_mode(ref)
    deviationFunction = get_deviation_function(refMode)

    E_Mev = Ekev / 1e3
    CSDA_rangeGCm2 = CSDA_range(E_Mev, Z, A, I_EV)
    CSDA_rangeCm = CSDA_rangeGCm2 / density
    CSDA_rangeUm = CSDA_rangeCm * 1e4

    rangeUm = deviationFunction(E_Mev, Z) * CSDA_rangeUm
    return rangeUm


def CSDA_range(E_Mev, Z, A, I_EV):
    ...
    CSDA_rangeGCm2 = c[1] / B * (
        np.log(1+c[2]*t0**c[3])/c[2] -
        c[4]*t0**c[5]/(1 + c[6]*t0)
    )
    return CSDA_rangeGCm2


def extrapolated_rangeSi(E_kev, ref="2002"):
    E_kev = float(E_kev)
    return extrapolated_range(E_kev, 14, 28.085, 171, 2.329, ref)
...
def extrapolated_range_xe(E_kev, p_bar, ref="2002"):
    ...
    A = 131.29
    I_EV = 482  # NIST ESTAR
    return extrapolated_range(E_kev, Z, A, I_EV, density, ref)
