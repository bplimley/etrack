...
def reconstruct(original_image_kev,
    ...
    output = TrackOutput()
    edge_segments = choose_initial_end(prepared_image_kev, options, output)
...
def construct_energy_kernel(radius_um, pixel_size_um):
    ...
    ceil_radius_pix = np.ceil(radius_pix) - 1
...
def choose_initial_end(image_kev, options, output):
    """
    """

    def locate_all_ends(image_kev, options, output):
        """
        """
        ends_image = [False]
        ...
        output.thinned_image = thinned_image
        ...
        if np.any(ends_image):
            ...
            output.ends_xy = ends_xy
        else:
            # still no ends.
            # Rather than raising an exception here, allow this to return.
            output.threshold_used = np.nan
            output.ends_xy = [[],[]]
            output.error = 'no ends found'


    def measure_energies(image_kev, options, output):
        """
        """
        ...
        output.ends_energy = np.array(ends_energy)


    def get_starting_point(options, output):
        """
        """
        min_index = output.ends_energy.argmin()
        # ...

    # main
    locate_all_ends(image_kev, options, output)
    if output.error == 'no ends found':
        raise NoEndsFound

    measure_energies(image_kev, options, output)
    get_starting_point(options, output)
